...
class MajorSignal(BinanceUtils):
    ...
    def major_signal(self) -> None:
        ...
        if not float(reversed_pos['positionAmt']):
            ...
            if not balance * self.config[f"{self.position_side.lower()}_positions_amount"] / 100 >= total_positions_amount:
                logger.info(
        ...
        order_id = self.create_market_order(symbol=self.symbol, qty=total_qty, side=self.side,
                                            pos_type=f'major_{self.position_side}', position_side=self.position_side)
